...
class MembershipsController < ApplicationController
  ...
  def webhooks
    ...
    render body: nil, status: webhook.process
  end
  ...
  def admin_grant_membership
    ...
    if @membership.save
      ...
      redirect_to new_user_membership_path(@user), flash: { alert: @membership.errors.full_messages.to_sentence }
    end
  end
  ...
  def prepare_new_form
    ...
    @cards = @user.stripe_customer&.list_payment_methods(type: 'card')&.data
  end

  # Strong params for +Membership+
  def membership_params
    params.require(:membership).permit(:user_id, :year, :type, :stripe_card_token, :subscribe, privileges: []).tap do |allowlisted|
