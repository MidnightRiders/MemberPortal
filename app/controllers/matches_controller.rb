class MatchesController < ApplicationController
  ...
  def index
    ...
    @matches = Match.includes(:pick_ems).where('kickoff >= :start_date AND kickoff <= :end_date', start_date: @start_date, end_date: @start_date + 7.days).order('kickoff ASC')
  end

  # GET /matches/1
  # GET /matches/1.json
  def show
    @mot_ms = Player.includes(:motm_firsts,:motm_seconds,:motm_thirds).select{|x| x.mot_m_total && x.mot_m_total > 0 }.sort_by(&:mot_m_total)
  end
