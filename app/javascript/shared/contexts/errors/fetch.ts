...
import { del, type Fetcher, type FetchError, type FetchOptions, get, patch, post, put } from '~helpers/fetch';

import { useAuthCtx } from '../auth';

import { type ErrorOptions, useErrorsCtx } from '.';
...
export interface FetchHook {
  (name: string): <T>(url: `/api/${string}`, data?: unknown, options?: FetchOptions) => Promise<T | false>;
  (
    name: string,
    options: Options,
    deps: unknown[],
  ): <T>(url: `/api/${string}`, data?: unknown, options?: FetchOptions) => Promise<T | false>;
}

const createFetchHook =
  (fetcher: Fetcher): FetchHook =>
  (name: string, { local, flash, capture, customMessage, ignore = false }: Options = {}, deps: unknown[] = []) => {
    ...
    return useCallback(
      async <T>(url: string, data?: unknown, opts?: FetchOptions) => {
        try {
          ...
          const resp = await fetcher<T & { jwt?: string | null }>(url, data, fetchOptions);
          const { jwt: newJwt, ...body } = resp;
          if (newJwt !== undefined) setJwt(newJwt);
          return body as T;
        } catch (err) {
          if (ignore === false || (typeof ignore === 'function' && !ignore(err))) {
            const message = customMessage?.(err) ?? (err as FetchError).message;
            addError(name, message, { flash, local });
            if (capture === true || (typeof capture === 'function' && capture(err))) {
