...
class MatchDecorator < Draper::Decorator
  ...
  def rev_guess_button(size='small', o = false)
    h.link_to h.rev_guess_path_for(model), class: "button secondary #{size}" do
      opp = o ? opponent : ''
      h.fa_icon('question fw') + ' RevGuess ' + opp + (": #{h.rev_guess_for(model)}" if h.rev_guess_for(model))
    end
  end
  ...
  def mot_m_button(size='small', o = false)
    ...
    h.link_to h.mot_m_path_for(model), class: "button secondary #{size}", title: "Man of the Match #{opp}" do
      h.fa_icon('list-ol fw') + ' MotM ' + opp + (h.fa_icon('check fw') if h.mot_m_for(model))
    end
  end
  ...
  def admin_controls
    h.capture do
      h.concat h.link_to(h.fa_icon('pencil fw'), h.edit_match_path(model), title: 'Edit') if h.can? :edit, model
      h.concat h.link_to(h.fa_icon('trash-o fw'),  model, method: :delete, data: { confirm: 'Are you sure?' }, title: 'Destroy') if h.can? :destroy, model
    end
  end
