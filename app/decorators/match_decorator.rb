...
class MatchDecorator < Draper::Decorator
  ...
  def rev_guess_button(size = 'small', o = false)
    h.link_to h.rev_guess_path_for(model), class: "button #{size}" do
      opp = o ? opponent : ''
  ...
  def mot_m_button(size = 'small', o = false)
    opp = o ? opponent : ''
    h.link_to h.mot_m_path_for(model), class: "button #{size}", title: "Man of the Match #{opp}" do
  ...
  def pick_em_button(pick, *args)
    ...
    if model.in_past?
      ...
      h.link_to(
        content,
        h.match_pick_em_vote_path(model),
        data: {
          remote: true,
          method: :post,
          params: {
