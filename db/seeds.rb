...
CSV.foreach(Rails.root.join('lib/assets/clubs.csv'), headers: true, header_converters: :symbol) do |club|
  Club.unscoped.find_or_initialize_by(abbrv: club[:abbrv]).update!(club.to_h)
end
puts ' done.'
...
unless Rails.env.test?
  ...
  CSV.foreach(Rails.root.join('lib/assets/players.csv'), headers: true, header_converters: :symbol) do |player|
    Player.unscoped.find_or_initialize_by(first_name: player[:first_name], last_name: player[:last_name]).update(p.to_h.merge(club_id: ne_id))
  end
  puts ' done.'
end
